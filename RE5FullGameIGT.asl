...
gameTime
{
	...
	if(settings["1.1.0"]){
		if(current.newIGT > old.newIGT){
			return TimeSpan.FromSeconds(System.Math.Floor(vars.totalGameTime + current.newIGT));
		}
	...
	if(settings["DX10"]){
		if(current.igtGFWL > old.igtGFWL){
			return TimeSpan.FromSeconds(System.Math.Floor(vars.totalGameTime + current.igtGFWL));
		}
